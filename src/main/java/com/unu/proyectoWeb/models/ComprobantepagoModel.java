...
public class ComprobantepagoModel extends Conexion {
	...
	public int insertarComprobante(Comprobantepago cp) throws SQLException {
		try {
			int filasAfectadas = 0;

			String sql = "CALL sp_comprobantePago(?,?,?)";
			this.abrirConexion();
			cs = conexion.prepareCall(sql);
			cs.setString(1, cp.getIdmediopago());
			cs.setString(2, cp.getIdtipoComprobante());
			cs.setString(3, cp.getNumero());
